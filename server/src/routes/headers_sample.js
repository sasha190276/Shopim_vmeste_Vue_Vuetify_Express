...
router.get('/headersGoogleSheetsWithRepliesAboutPayments', async (req, res) => {
  const tableConfig = {};
  tableConfig.headersOfTable = {};
  tableConfig.depForChangeValueHeaders = [];
  const headers = await db.getDb().collection('headersGoogleSheetsWithRepliesAboutPayments').find({}, { fields: { _id: 0 } }).toArray();
